#!/usr/bin/env python3
"""Run a collaborator-style structured DFL benchmark on an enumerated shortest-path toy task.
...
5) exports CSV, TeX, figures, and a Chinese markdown summary.
"""
...
def make_summary_table(result_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Single-seed debug runs yield NaN std values; use zero-width error bars instead.
    std_columns = [
        "standard_spo_loss_std",
        "path_accuracy_std",
        "edge_overlap_std",
        "runtime_std",
    ]
...
def main() -> None:
    ...
    # Save figures and summaries.
    save_figures(out_dir, history_df, summary_df)
    summary_md = build_summary_markdown(config, selected_df, display_df, summary_df)
    (out_dir / "summary_zh.md").write_text(summary_md, encoding="utf-8")

    # Repo helper files.
    (out_dir / "README.md").write_text(build_readme(), encoding="utf-8")
    (out_dir / "Makefile").write_text(build_makefile(), encoding="utf-8")
